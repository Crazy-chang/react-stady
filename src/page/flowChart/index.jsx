  ...
  const graphData = {
    nodes: [
      // 节点
      {
        ...
        x: 200,
        y: 100,
        text: { x: 200, y: 40, value: "节点1" }, // 自定义名称位置
        properties: {
          //  属性自定义节点样式
          isPass: true,
        },
    ...
    edges: [
      // 边/线
      {
        ...
        text: { x: 200, y: 250, value: "连线" },
        startPoint: { x: 140, y: 140 }, // 线起点
        endPoint: { x: 300, y: 400 }, // 线终点
        pointsList: [
          // 折线连线的点位置
          { x: 200, y: 140 },
          { x: 250, y: 200 },
  ...
  useEffect(() => {
    const configLf = {
      ...
      grid: {
        size: 20,
        visible: true,
        type: "mesh", // 目前支持 dot 点状和 mesh 线状两种
        config: {
          color: "#ababab",
          thickness: 1,
        },
      ...
      width: 1200,
      height: 700,
      plugins: [Menu, DndPanel, SelectionSelect, Control, MiniMap],
    };
    const logicflow = new LogicFlow(configLf);
    ...
    logicflow.extension.control.addItem({
      ...
      title: "导-航",
      text: "导航",
      onMouseEnter: (logicflow, ev) => {
        const position = logicflow.getPointByClient(ev.x, ev.y);
        logicflow.extension.miniMap.show(
          position.domOverlayPosition.x - 120,
          position.domOverlayPosition.y + 35
        );
      },
      onClick: (logicflow, ev) => {
        const position = logicflow.getPointByClient(ev.x, ev.y);
        logicflow.extension.miniMap.show(
          position.domOverlayPosition.x - 120,
          position.domOverlayPosition.y + 35
  ...
  return (
    <div>
      <Button
        type="primary"
        onClick={() => {
          console.log("获取流程图数据：", lf.getGraphRawData());
        }}
      >
        获取JSON
      </Button>
